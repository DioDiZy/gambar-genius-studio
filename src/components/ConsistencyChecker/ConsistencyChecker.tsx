    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button size="sm" variant="outline" onClick={exportData} className="gap-1">
          ...
          {!showReset ? (
            <Button size="sm" variant="destructive" onClick={() => setShowReset(true)} className="gap-1">
      ...
      <div className="flex gap-2 flex-wrap">
        {issues.length === 0 ? (
          <Badge className="bg-green-100 text-green-700 gap-1">
            <CheckCircle size={12} /> Semua Konsisten
          </Badge>
        ) : (
          <>
            {broken > 0 && (
              <Badge variant="destructive" className="gap-1">
                <XCircle size={12} /> {broken} Broken Reference
              </Badge>
            )}
            {missing > 0 && (
              <Badge className="bg-orange-100 text-orange-700 gap-1">
                <AlertTriangle size={12} /> {missing} Missing Data
              </Badge>
            )}
            {attention > 0 && (
              <Badge className="bg-yellow-100 text-yellow-700 gap-1">
                <Info size={12} /> {attention} Needs Attention
              </Badge>
            )}
          </>
        )}
