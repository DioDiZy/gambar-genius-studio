...
export const StoryboardAnalyzer = ({ storyboardData, isVisible }: StoryboardAnalyzerProps) => {
  if (!isVisible || !storyboardData) return null;
  ...
  const renderSceneMetadata = (scene: StoryboardPrompt) => (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <div className="text-xs text-muted-foreground">Nuansa</div>
    ...
    <Card className="mt-4">
      <CardHeader>
        ...
        <div className="flex gap-2">
          ...
          {metadata.enhancedFormat && <Badge variant="default">Format ditingkatkan</Badge>}
        </div>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="scenes" className="w-full">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="continuity">Kontinuitas</TabsTrigger>
            <TabsTrigger value="json">JSON Mentah</TabsTrigger>
          </TabsList>
                ...
                <div key={scene.sceneIndex} className="mb-6">
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <Separator />
                      <div>
                        <h5 className="font-medium mb-3">Metadata Adegan</h5>
              ...
              {structuredData?.map((scene) => (
                <div key={scene.sceneIndex} className="mb-6">
                  <h4 className="font-medium mb-3 flex items-center gap-2">
                    ...
                    Karakter Adegan {scene.sceneIndex + 1}
                  </h4>
                  {scene.characters.length > 0 ? (
                    <div className="space-y-3">{scene.characters.map(renderCharacterReference)}</div>
                  ) : (
                    <p className="text-sm text-muted-foreground italic">Tidak ada karakter yang teridentifikasi di adegan ini</p>
                  )}
              ...
              {structuredData?.map((scene, index) => (
                <div key={scene.sceneIndex} className="mb-6">
                  <h4 className="font-medium mb-3 flex items-center gap-2">
                    ...
                    Tautan Kontinuitas Adegan {scene.sceneIndex + 1}
                  </h4>
