 ...
 */
export function buildVisualMemory(
  sceneIndex: number,
  scenes: Array<{ paragraph: string; scene_type: string; location: string; characters: string[] }>,
  ...
  const previousScenesInSameLocation = scenes
    .slice(0, sceneIndex)
    .filter(scene => scene.location === currentLocation);
  ...
  let lightingContext = '';
  if (sceneIndex > 0) {
    const timeIndicators = scenes.slice(0, sceneIndex + 1)
 ...
 */
export async function createStoryboardPrompts(
  paragraphs: string[],
  characters: CharacterDescription[],
  style: string,
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    if (visualMemory.length > 0) {
      const memoryDescriptions = visualMemory
        .filter(vm => vm.consistency_weight > 0.5)
